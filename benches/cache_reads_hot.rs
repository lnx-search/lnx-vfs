...
fn main() {
    ...
    for &config in CONFIG {
        ...
        for page_len_range in PAGE_LEN_RANGES.iter().cloned() {
            let result = bench_hot_page_reads(config, page_len_range);
            results.push(result);
        }
    }
...
#[inline(never)]
#[unsafe(no_mangle)]
fn bench_hot_page_reads(
    config: BenchmarkConfig,
    page_len_range: Range<usize>,
) -> BenchmarkMetrics {
    let name = format!(
        "Hot Reads - page_size:{}, cache_size:{}",
        config.page_size,
    ...
    do_read_layer(&layer, 0..layer_num_pages as u32, &buffer);
    ...
    for _ in 0..NUM_RUNS {
        ...
        let result = black_box(do_read_layer(
            black_box(&layer),
